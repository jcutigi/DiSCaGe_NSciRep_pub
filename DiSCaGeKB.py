...
def get_oncogene_list(input_oncogene_file_name):
    oc_df = pd.read_csv(input_oncogene_file_name, sep='\t', index_col=0, header=None)
    oncogene_list = list(oc_df.index)
...
def get_weighted_mutation_matrix_std_og(maf, mutation_weights, mutation_weights_og, oncogene_list):
    patients = list(set(maf.Tumor_Sample_Barcode))
    genes = list(set(maf.Hugo_Symbol))
    mutation_types = list(set(maf.Variant_Classification))
    
    mutation_weights = {k: v for k, v in mutation_weights.items() if k in mutation_types}
    mutation_weights_og = {k: v for k, v in mutation_weights_og.items() if k in mutation_types}
    ...
    for p in patients:
        ...
        for g in genes:
            ...
            number_of_mutations = sum(mut_dict.values())
            if g not in oncogene_list:
                sum_product = sum(mut_dict[k] * mutation_weights[k] for k in mut_dict)
            else:
                sum_product = sum(mut_dict[k] * mutation_weights_og[k] for k in mut_dict)
            score = 0
            if number_of_mutations > 0:
                score =  round(sum_product / number_of_mutations, 3)
            wmm.at[p, g] = score
...
def main():
    ...
    if input_type == 4:
        input_oncogene_file_name = cp_input["ONCOGENE_FILE_NAME"]
        cp_mutation_weights = cp["VARIANT_CLASSIFICATION_WEIGHTS_OG"]
        mutation_weights_og = {}
        mutation_weights_og["Nonsense_Mutation"] = float(cp_mutation_weights["NONSENSE"])
        mutation_weights_og["Missense_Mutation"] = float(cp_mutation_weights["MISSENSE"])
        mutation_weights_og["Splice_Site"] = float(cp_mutation_weights["SPLICE"])
        mutation_weights_og["Frame_Shift_Del"] = float(cp_mutation_weights["FRAMESHIFT_DEL"])
        mutation_weights_og["Frame_Shift_Ins"] = float(cp_mutation_weights["FRAMESHIFT_INS"])
        mutation_weights_og["In_Frame_Del"] = float(cp_mutation_weights["INFRAME_DEL"])
        mutation_weights_og["In_Frame_Ins"] = float(cp_mutation_weights["INFRAME_INS"])
        mutation_weights_og["3'UTR"] = float(cp_mutation_weights["_3UTR"])
        mutation_weights_og["5'UTR"] = float(cp_mutation_weights["_5UTR"])
        mutation_weights_og["Nonstop_Mutation"] = float(cp_mutation_weights["NONSTOP_MUTATION"])
        mutation_weights_og["Translation_Start_Site"] = float(cp_mutation_weights["TRANSLATION_START_SITE"])
    ...
    if input_type == 1 or input_type == 4:
        ...
        if input_type == 1:
            ...
            oncogene_list = get_oncogene_list(input_oncogene_file_name)
            wmm = get_weighted_mutation_matrix_std_og(maf, mutation_weights, mutation_weights_og, oncogene_list)
